...
handle_event({wtt, Callback, Id}) ->
    ...
    wf:update(wtts, wtts());
handle_event({click_card, Wtt})->
    io:format("DU KLICKADE!!!",[]),
    Show = case wf:state(Wtt#wtts.id) of
        undefined ->                    
            wf:state(Wtt#wtts.id, true),
            [ #p{}, get_user_list("Wanters: ", Wtt#wtts.wanters),
              get_user_list("Havers: ", Wtt#wtts.havers)];
        false ->   
            wf:state(Wtt#wtts.id, true),
            [ #p{},get_user_list("Wanters: ", Wtt#wtts.wanters),
              get_user_list("Havers: ", Wtt#wtts.havers)];
        ...
        true ->
            wf:state(Wtt#wtts.id, false),
            []
    end, 

 
    wf:update(placeholder, Show);
handle_event("666")->
    ok.
		    ...
		    true ->
			 ""
                 ...
                 #panel{id = Id,
                     class = "card",
	   body = [
           show_card(Card, Wtt, WantPB, HavePB),
           #panel{ id = placeholder}
         ]}.

show_card(Card, Wtt, WantPB, HavePB)->
    NumW = integer_to_list(length(Wtt#wtts.wanters)),
    NumH = integer_to_list(length(Wtt#wtts.havers)),
    W = #link{text = " W: "++NumW, postback = WantPB},
    H = #link{text = " H: "++NumH, postback = HavePB},
    [#p{},#link{text=Card#cards.name,postback={click_card, Wtt}}, W, H].
    
get_user_list(What, Wtt)->
    #list{body = [What, lists:map(
            fun(UserID) ->
		        User = umts_db:get_user(UserID),
...
tooltip(Prefix, Title, Wtt, Postback) ->
	    [#link{text = [Prefix, integer_to_list(length(Wtt))], postback = Postback},
